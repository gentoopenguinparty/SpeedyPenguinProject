...
export default function QuestionsAnswers({ productId, trackClick }) {
  ...
  const getQAs = () => axiosGet(`https://app-hrsei-api.herokuapp.com/api/fec2/:hr-rfe/qa/questions/?product_id=${productId}&count=50`).then((response) => {
  ...
  return (
    ...
    <MainContainer onClick={(e) => trackClick(e, 'QA')}>
      ...
      <h2 className="title" data-testid={'test'}>Questions & Answers</h2>
      <br />
      <Wrapper>
        {' '}
        <SearchBar className="searchBar" data-testid={'searchbar'} placeholder="Have A Question? Search For Answers..." onChange={(e) => setSearch(e.target.value)} />
      ...
      <br />
      {noMoreQs && <Button data-testid={'increment button'} onClick={incrementQList}>Load More Questions</Button>}
